            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    <CardHeader className="py-4 pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Demos Today</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{calendarData.demos.length}</div>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="py-4 pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Leads to Call</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold">{calendarData.assignedLeads.length}</div>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="py-4 pb-2">
                        <CardTitle className="text-sm font-medium text-muted-foreground">Pending Works</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-red-600">{calendarData.pendingWorksCount}</div>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="py-4 pb-2">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{calendarData.recentFollowUps.length}</div>
                    </CardContent>
                </Card>
            </div>
                        ...
                        {calendarData.assignedLeads.map((l: any, i: number) => (
                            <div key={`al-${i}`} className="flex justify-between items-center p-3 border rounded-md hover:bg-slate-50 transition">
                                ...
                                <div className="flex items-center gap-2">
                                    <Badge variant="outline" className="text-[10px] hidden sm:inline-flex">{l.status}</Badge>
                                    {l.phone && (
                                        <>
                                            <a href={`tel:${l.phone}`} className="p-1.5 bg-blue-100 text-blue-600 rounded hover:bg-blue-200 transition" title="Call">
                                                <PhoneCall className="h-3 w-3" />
                                            </a>
                                            <a href={`https://wa.me/${l.phone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-green-100 text-green-600 rounded hover:bg-green-200 transition" title="WhatsApp">
