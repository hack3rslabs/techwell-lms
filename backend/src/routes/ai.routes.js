 ...
 * @desc    Context-aware AI Tutor for Course Player
 * @access  Private
 */
router.post('/doubt-solver', authenticate, async (req, res, next) => {
    try {
        const { message, history, courseTitle, lessonTitle, lessonContent } = req.body;

        if (!message) return res.status(400).json({ error: 'Message is required' });

        const user = req.user;
        const systemRole = `You are "Techwell Tutor", an elite AI teaching assistant for techwell.co.in.
Your goal is to help the student understand the concepts without just giving them the direct answer if it's a quiz.
...
2. Format answers beautifully with bullet points if necessary.
3. If the student asks something completely off-topic (e.g. politics, unrelated coding), politely steer them back to the lesson.
4. Keep responses under 4-5 paragraphs.`;
        ...
        if (process.env.GEMINI_API_KEY) {
            try {
                ...
                const chat = model.startChat({
                    history: (Array.isArray(history) ? history : [])
                        .filter(h => h && typeof h === 'object' && h.parts && h.parts.length > 0)
                        .map(h => ({
                            role: h.role === 'assistant' || h.role === 'model' ? 'model' : 'user',
                            parts: [{ text: String(h.parts[0].text) }]
                        })),
                ...
                console.error("Gemini Tutor Error:", aiError);
                aiResponse = "I'm having trouble connecting to my AI brain. Please try again!";
            }
        } else {
            aiResponse = `[MOCK TUTOR] Hello ${user.name}! You asked about "${lessonTitle}". I am simulated because the API key is missing.`;
        }
