...
export function PermissionGuard({ children, featureCode }: PermissionGuardProps) {
    ...
    const getFeatureFromPath = (path: string) => {
        ...
        if (path.startsWith("/admin/crm")) return "CENTRAL_CRM"
        ...
        if (path.startsWith("/admin/team")) return "TEAM_MANAGEMENT"
        if (path.startsWith("/admin/cms")) return "CMS"
        if (path.startsWith("/admin/marketing")) return "MARKETING_HUB"
