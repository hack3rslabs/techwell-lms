...
export default function JobDetailPage() {
    ...
    const [aiMatch, setAiMatch] = useState<{ matchScore: number | null, rationale: string } | null>(null)
    ...
    useEffect(() => {
        const fetchJobData = async () => {
            try {
                ...
                // Fetch AI match if student
                if (user?.role === 'STUDENT') {
                    const matchRes = await api.get(`/jobs/${params.id}/student-match`)
    ...
    const handleGenerateCoverLetter = async () => {
        ...
        try {
            const res = await api.post(`/jobs/${params.id}/generate-cover-letter`)
            setApplicationForm(prev => ({ ...prev, coverLetter: res.data.coverLetter }))
        } catch (error: any) {
                                            ...
                                            <div className="flex justify-between items-center">
                                                ...
                                                {user?.role === 'STUDENT' && (
                                                    ...
                                                    >
                                                        ...
                                                        {isGeneratingCL ? 'Generating...' : 'Auto-Draft AI Cover Letter'}
                                                    </Button>
                                                )}
                                            </div>
                                            <Textarea
                        ...
                        <Card className="border-indigo-100 bg-indigo-50/50 shadow-sm">
                            ...
                            <CardContent>
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-slate-700 font-medium">{aiMatch.rationale}</p>
                                        <p className="text-xs text-muted-foreground mt-1">Based on your ATS Resume Profile</p>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
